 ...
 * @generated-id: 757ef899b128
 */

import { PumbleSDKDefaultError } from "../models/errors/pumble-sdk-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | PumbleSDKDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | PumbleSDKDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new PumbleSDKDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new PumbleSDKDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
