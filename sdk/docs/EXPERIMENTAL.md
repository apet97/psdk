...
`pumble-sdk/extensions/app/index.js` exposes building blocks for a Pumble app install flow, but does **not** yet provide:
...
`pumble-sdk/extensions/app/socket-mode.js` requires an injected WebSocket implementation. There is no first-class reconnect/heartbeat policy bundled. Live tests are not part of the release gate.
