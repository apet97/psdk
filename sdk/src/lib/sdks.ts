...
import { redactDebugHeaders, redactDebugValue } from "../extensions/debug-redaction.js";
...
export class ClientSDK {
  ...
  constructor(options: SDKOptions = {}) {
    ...
    if (!this.#logger && env().PUMBLESDK_DEBUG) {
      this.#logger = console;
    }
  }
...
async function logRequest(logger: Logger | undefined, req: Request) {
  ...
  switch (true) {
    case jsonLikeContentTypeRE.test(ct):
      logger.log(redactDebugValue(await req.clone().json()));
      break;
    case ct.startsWith("text/"):
      logger.log(redactDebugValue(await req.clone().text()));
      break;
    case ct === "multipart/form-data": {
      const body = await req.clone().formData();
      for (const [k, v] of body) {
        const vlabel = v instanceof Blob ? "<Blob>" : redactDebugValue(v, k);
        logger.log(`${k}: ${vlabel}`);
      }
      break;
    }
...
async function logResponse(
  ...
  switch (true) {
    case matchContentType(res, "application/json")
      ...
      logger.log(redactDebugValue(await res.clone().json()));
      break;
    ...
    case matchContentType(res, "text/*"):
      logger.log(redactDebugValue(await res.clone().text()));
      break;
